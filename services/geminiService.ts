...
        model: 'imagen-3.0-generate-002', // ИСПРАВЛЕНО: Используем рабочую модель
        prompt: fullPrompt,
        config: {
...
    const ai = new GoogleGenAI({ apiKey: effectiveApiKey });
    
    if (model === 'imagen-3.0-generate-002') { // ИСПРАВЛЕНО (на всякий случай, если constants.ts передаст новое имя)
        const response = await ai.models.generateImages({
            model: 'imagen-3.0-generate-002', // ИСПРАВЛЕНО: Используем рабочую модель
            prompt: prompt,
            config: {
...
    } else {
        // ... (остальная логика для Nano Banana)
        const parts = [];
